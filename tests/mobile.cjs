  ...
  try {
    ...
    for (const viewport of [{width:320,height:568},{width:390,height:844},{width:568,height:320}]) {
      ...
      for (const q of data.questions) {
        ...
        const noticePattern = { unverified: /尚未核对原卷/, 'teaching-example': /本站编写的教学示例/, 'exam-confirmed': /维护者确认为高考真题.*未附官方原卷链接/, 'bank-original': /确认为高考真题.*与维护者自有题库.*一致.*未附官方原卷链接/, 'bank-adapted': /据高考真题改编.*维护者自有题库.*未附官方原卷链接/ }[q.source.status];
        ...
        if (q.source.originalLabel) {
          ...
          assert(!/官方核验|官方原卷已核验|官方原卷一致|原卷原题|已核对原卷/.test(dialogText), `${q.id}: 没有官方材料时不应出现官方核验类措辞`);
        }
      ...
      for (const q of data.questions) {
        const expected={ unverified:/出处待核对/, 'teaching-example':/本站教学示例/, 'exam-confirmed':/高考真题 · 维护者确认/, 'bank-original':/高考真题 · 与维护者题库一致/, 'bank-adapted':/高考真题改编 · 据维护者题库整理/ }[q.source.status];
