...
assert.equal(data.questions.filter(q=>q.source.status==='unverified').length, 7, '待核对题目数量变化须人工复核（2026-09-25：题库核对 16 → 13，维护者确认高考来源 13 → 7）');
assert.equal(data.questions.filter(q=>q.source.status==='exam-confirmed').length, 6, '维护者确认高考真题的题目数量变化须人工复核');
...
assert.equal(data.questions.length, 53, '例题总数变化须人工复核');
